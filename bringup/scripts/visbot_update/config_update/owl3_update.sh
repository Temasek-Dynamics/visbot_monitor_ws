...
if [ "$choice" -eq 1 ]; then
  drone_setting="owl3_customized"
elif [ "$choice" -eq 0 ]; then
  drone_setting="owl3_factory"
else
...
drone_dir="$SCRIPT_DIR/owl3/$drone_setting"
drone_yaml="$drone_dir/owl3_config.yaml"
...
for i in $(yq eval '.owl3 | keys' $drone_yaml | grep -o '[0-9]\+'); do
  
  drone_id=$(yq eval ".owl3[$i].Drone_ID" $drone_yaml)

  # Drone parameters
  swarm_id=$(yq eval ".owl3[] | select(.Drone_ID == $drone_id) | .drone_id" $drone_yaml)
  swarm_num=$(yq eval ".owl3[] | select(.Drone_ID == $drone_id) | .drone_num" $drone_yaml)
  drone_ip=$(yq eval ".owl3[] | select(.Drone_ID == $drone_id) | .IP" $drone_yaml)
  tgt_system=$(yq eval ".owl3[] | select(.Drone_ID == $drone_id) | .tgt_system" $drone_yaml)
  MAV_SYS_ID=$(yq eval ".owl3[] | select(.Drone_ID == $drone_id) | .MAV_SYS_ID" $drone_yaml)
  gcs_url=$(yq eval ".owl3[] | select(.Drone_ID == $drone_id) | .gcs_url" $drone_yaml)

  echo "-------------------------------------------------------------------------------"
  echo -e "Updating configuration parameters for OWL 3 Drone $drone_id ($drone_ip)"
