...
config_path=$(rospack find $ros_package)/config/mission_loop
...
# roslaunch $ros_package sequential_waypoint_loop.launch yaml_file:="$config_path/$mission_file.yaml" > >(tee /dev/stderr) 2>/dev/null
roslaunch $ros_package sequential_waypoint_loop.launch yaml_file:="$config_path/$mission_file.yaml"