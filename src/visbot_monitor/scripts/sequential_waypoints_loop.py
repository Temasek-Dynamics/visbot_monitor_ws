...
class SequentialWpsMission:
    def __init__(self):
        ...
        swarm_matrix = swarm_config.get("drones", [])
        ...
        self.swarm_matrix.x = [drone['position']['x'] for drone in swarm_matrix]
        self.swarm_matrix.y = [drone['position']['y'] for drone in swarm_matrix]
        self.swarm_matrix.z = [drone['position']['z'] for drone in swarm_matrix]
        self.swarm_matrix.yaw = [drone['yaw'] for drone in swarm_matrix]
...
if __name__ == '__main__':
    ...
    print_warning("The drone has started a loop flight \n")
    rospy.init_node('seq_wps_loop', anonymous=False)
    ...
    while not rospy.is_shutdown():
        while seq_wps_m.is_done is False:
            if seq_wps_m.current_wp < len(seq_wps_m.wps_msg_list):
                ...
                print("All waypoints sent")
                manual_done = str_to_bool(input("Is the mission done? (yes/no): "))
                if manual_done:
                    seq_wps_m.is_done = True
                    print("Mission completed")
                    break
                else:
                    print("Restart from the first waypoint \n")
                    seq_wps_m.current_wp = 0
